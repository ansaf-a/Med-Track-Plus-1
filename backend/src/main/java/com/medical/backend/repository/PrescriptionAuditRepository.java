package com.medical.backend.repository;

import com.medical.backend.entity.PrescriptionAudit;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;
import java.util.List;

@Repository
public interface PrescriptionAuditRepository extends JpaRepository<PrescriptionAudit, Long> {
    List<PrescriptionAudit> findByPrescriptionIdOrderByModifiedAtDesc(Long prescriptionId);

    List<PrescriptionAudit> findByPrescriptionIdOrderByModifiedAtAsc(Long prescriptionId);

    long countByPrescriptionId(Long prescriptionId);
}
